...
def salvar_no_banco(nome, caminho_imagem):
    imagem = face_recognition.load_image_file(caminho_imagem)
    codificacoes = face_recognition.face_encodings(imagem)
    if codificacoes:
        embedding_json = json.dumps(codificacoes[0].tolist())
        sql = "INSERT INTO rostos (nome, embedding, imagem_path) VALUES (%s, %s, %s)"
        cursor.execute(sql, (nome, embedding_json, caminho_imagem))
        conexao.commit()
        print(f"[OK] Embedding de {nome} salvo no banco.")
    else:
        print(f"[ERRO] Não foi possível gerar embedding para {nome}.")
